...
coconut::rabbitmq::Config::Config(const toml::table &node) {
...
coconut::rabbitmq::Client::Client(const coconut::rabbitmq::Config &config) {
...
coconut::rabbitmq::Client::~Client() {
  {
    const auto reply =
...
void coconut::rabbitmq::Client::declare_exchange(const std::string &name,
...
void coconut::rabbitmq::Client::declare_queue(const std::string &name,
                                              bool durable) {
...
void coconut::rabbitmq::Client::bind_queue(const std::string &queue,
...
void coconut::rabbitmq::Client::publish(const std::string &exchange,
                                        const std::string &routing_key,
                                        const std::string &content_type,
                                        const std::string &message) {
  const std::string &id = coconut::uuid();
...
static void _consume_message(amqp_connection_state_t connection,
                             coconut::rabbitmq::Handler &handler) {
  amqp_frame_t frame;
  for (;;) {
    ...
    if ((envelope.message.properties._flags & AMQP_BASIC_CONTENT_TYPE_FLAG) &&
      ...
      amqp_basic_ack(connection, coconut::rabbitmq::Client::CHANNEL_ID,
...
void coconut::rabbitmq::Client::consume(const std::string &name,
