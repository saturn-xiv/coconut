...
class EchoAmqpHandler : public coconut::rabbitmq::Handler {
 public:
...
TEST_CASE("RabbitMQ", "[rabbitmq]") {
  ...
  const coconut::rabbitmq::Config cfg(*node);
  std::cout << "connect " << cfg << std::endl;
  coconut::rabbitmq::Client cli(cfg);
  ...
  SECTION("setuo queue & exchange") {
    ...
    cli.declare_exchange(exchange, coconut::rabbitmq::Client::EXCHANGE_TYPE_DIRECT,
                         true);
