...
class Health_version_args {
 public:

  Health_version_args(const Health_version_args&) noexcept;
  Health_version_args& operator=(const Health_version_args&) noexcept;
  Health_version_args() noexcept {
  }

  virtual ~Health_version_args() noexcept;

  bool operator == (const Health_version_args & /* rhs */) const
  {
    return true;
  }
  bool operator != (const Health_version_args &rhs) const {
  ...
  bool operator < (const Health_version_args & ) const;
...
class Health_version_pargs {
 public:


  virtual ~Health_version_pargs() noexcept;

  uint32_t write(::apache::thrift::protocol::TProtocol* oprot) const;

};

typedef struct _Health_version_result__isset {
  _Health_version_result__isset() : success(false) {}
  bool success :1;
} _Health_version_result__isset;

class Health_version_result {
 public:

  Health_version_result(const Health_version_result&);
  Health_version_result& operator=(const Health_version_result&);
  Health_version_result() noexcept
                        : success() {
  }

  virtual ~Health_version_result() noexcept;
  std::string success;

  _Health_version_result__isset __isset;

  void __set_success(const std::string& val);

  bool operator == (const Health_version_result & rhs) const
  {
  ...
  bool operator < (const Health_version_result & ) const;
...
typedef struct _Health_version_presult__isset {
  _Health_version_presult__isset() : success(false) {}
  bool success :1;
} _Health_version_presult__isset;

class Health_version_presult {
 public:


  virtual ~Health_version_presult() noexcept;
  std::string* success;

  _Health_version_presult__isset __isset;
...
class HealthProcessor : public ::apache::thrift::TDispatchProcessor {
 ...
 public:
  HealthProcessor(::std::shared_ptr<HealthIf> iface) :
    ...
    processMap_["version"] = &HealthProcessor::process_version;
  }
